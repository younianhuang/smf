...
void ParseSPS(unsigned char * pStart, unsigned short nLen);
...
void continueAfterDESCRIBE(RTSPClient*, int resultCode, char* resultString) {
  ...
  *env << "Opened URL \"" << streamURL << "\", returning a SDP description:\n" << sdpDescription << "\n";
  ...
  std::string sps(sdpDescription);
  size_t pos1 = sps.find("sprop-parameter-sets=") + std::string("sprop-parameter-sets=").length();
  size_t pos2 = sps.find(",", pos1);
  sps.assign(sps, pos1, pos2 - pos1);

  // Base64 to Base16
  const char *sps_sdp = sps.c_str();
  std::cout << "sps_sdp = " << sps_sdp << "\n";
  ...
  const char* singleMediumToTest = singleMedium;
  while ((subsession = iter.next()) != NULL) {

    const char* mediumName = subsession->mediumName();

    std::cout << "mediumName = " << mediumName << ", " <<
                 "codecName = " << subsession->codecName() << std::endl;

    if((strcmp(mediumName, "video") == 0) ||
       (strcmp(mediumName, "audio") == 0))
    {
        numOfStream++;
    }
...
//        streamSources.push_back(pRTSPVideoStreamSource);
//    }
//    else
//    {
//        streamSources.push_back(nullptr);
//    }
...
void createOutputFiles(char const* periodicFilenameSuffix) {
  ...
  if (outputQuickTimeFile || outputAVIFile) {
	...
	*env << "Outputting to the file: \"" << outFileName << "\"\n";
      ...
      aviOut = SMFAVIFileSink::createNew(*env, *session, outFileName,
				      fileSinkBufferSize,
				      movieWidth, movieHeight,
				      movieFPS,
				      packetLossCompensate);
      if (aviOut == NULL) {
    *env << "Failed to create an \"SMFAVIFileSink\" for outputting to \""
	     << outFileName << "\": " << env->getResultMsg() << "\n";
     _shutdown();
      } else {
	*env << "Outputting to the file: \"" << outFileName << "\"\n";
...
//  watchVariable = 1; // Return from env->taskScheduler().doEventLoop()
}
...
void ParseSPS(unsigned char * pStart, unsigned short nLen)
{
    m_pStart = pStart;
    m_nLength = nLen;
    m_nCurrentBit = 0+8; // Skip first byte (sps = 67, pps = 68)
...
//     -4 -P 10 -d 1 -f 30 -u admin admin -b 200000 rtsp://172.26.12.55/stream1
//                                                  rtsp://184.72.239.149/vod/mp4:BigBuckBunny_175k.mov
//
// VLC Streaming Server:
//     cd /opt/live/testProgs
//     cp /opt/SurveonMediaPlayer/bin/resource/test1080p.avi test.264
//     ./testH264VideoStreamer
//     vlc rtsp://172.26.12.27:8554/testStream
